...
export default function Reservation() {
  ...
  const {
    brand, location, image, payment, price,
  } = detail.vehicle;
  const countPlus = () => {
  ...
  return (
    <div className="vehicle-detail">
      <section className="container first-section reservation mb-5">
        <div onClick={back} style={{ cursor: 'pointer' }} className="d-flex flex-row head" aria-hidden="true">
              ...
              <div className="col-12 col-lg-5 description-section">
                ...
                <div className="my-auto">
                  <div className="total-day d-flex flex-row justify-content-between align-items-center">
                    <button onClick={countMinus} className="btn" aria-label="button minus" type="button"><BiMinus className="minus" /></button>
                    <div className="count">{counter.totalItem}</div>
                    <button onClick={countPlus} className="btn" aria-label="button plus" type="button"><BiPlus className="plus" /></button>
                  </div>
                </div>
                <form>
        ...
        <div className="pay-now mt-5 px-2">
          <button onClick={gotoPayment} className="btn btn-green w-100 mt-3" type="button">
            Pay now: Rp.
            <span>
              {formatPrice}
            </span>
